class DashboardController < ApplicationController
  ...
  def index
    ...
    if params[:refresh]=="hard_refresh"
      ...
      @response_calories = FetchData.get_datastreams(session,source="google-fit",datatype="com.personicle.individual.datastreams.interval.total_calories",start_date=st, end_date=et, hard_refresh=true,uid=session[:oktastate]['uid'])
      ...
      @response_calories = FetchData.get_datastreams(session,source="google-fit",datatype="com.personicle.individual.datastreams.interval.total_calories",start_date=st, end_date=et, hard_refresh=false,uid=session[:oktastate]['uid'])
      # puts "Step data"
      # puts @response_step
    end 
    ...
    if !@response_step.empty?
      ...
      tmp_steps = @response_step.select {|record| record['end_time'].to_datetime > 30.days.ago}.map {|rec| [rec['end_time'].to_date, rec['value']]}.group_by {|r| r[0]}.to_h
